...
export default function BuyCrypto() {
  ...
  const [premium, setPremium] = useState<boolean>(true);  // New state for premium status
  ...
  const fetchUserDetails = async (uid: string) => {
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        if (userData.Premium === false || userData.Premium === undefined) {
          setPremium(false);
        } else {
          setPremium(true);
        }
  ...
  const handleBuyNow = async (crypto: Crypto) => {
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        if (currentCredit >= totalCost) {
          ...
          await addDoc(portfolioRef, {
            AssetID: crypto.symbol, // Use stock ticker as AssetID
            PortfolioID: portfolioID, // Randomly generated PortfolioID
            PurchaseDate: new Date(), // Current date and time
            PurchasePrice: crypto.price, // Current price of the stock
            Quantity: quantity,
            totalCost: totalCost,
          ...
          Alert.alert('Insufficient Credit', 'You do not have enough credit to buy this stock.');
        }
      } else {
      ...
      Alert.alert('Error', 'Unable to complete the purchase. Please try again.');
    }
  };
      ...
      {selectedCrypto && (
        <View style={styles.modalContent}>
          ...
          <View style={styles.stockInfo}>
            ...
            <TouchableOpacity 
        style={[styles.recommendationButton, !premium && styles.disabledButton]} 
        onPress={premium ? makeRecommendation : undefined}
        disabled={!premium}
        onLongPress={() => {
          if (!premium) {
            Alert.alert('Premium Required', 'You need a premium account to make recommendations.');
          }
        }}
      >
